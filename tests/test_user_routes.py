...
def test_login_as_admin():
    ...
    response = client.post("/user/login", json= {"username": ADMIN_USERNAME, "password": ADMIN_PASSWORD})
    assert response.status_code == status.HTTP_200_OK
    ...
    secured_response = client.get("/secured", headers={"Authorization": f"Bearer {access_token}"})
