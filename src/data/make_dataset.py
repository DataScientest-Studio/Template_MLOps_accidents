...
# ---------- Functions without saves -----------------------------------------
def make_dataset_without_saving(input_filepath, output_filepath):
    """ Runs data processing scripts to turn raw data from (../interim) into
        cleaned data ready to be analyzed (saved in ../preprocessed).
    """
    logger = logging.getLogger(__name__)
    ...
    input_filepath_users = os.path.join(input_filepath, "eval_usagers.csv")
    input_filepath_caract = os.path.join(input_filepath,
                                         "eval_caracteristiques.csv")
    input_filepath_places = os.path.join(input_filepath, "eval_lieux.csv")
    input_filepath_veh = os.path.join(input_filepath, "eval_vehicules.csv")

    # Call the main data processing function with the provided file paths
    process_data_without_saving(input_filepath_users,
                                input_filepath_caract,
                                input_filepath_places,
                                input_filepath_veh,
                                output_filepath)
...
def process_data_without_saving(input_filepath_users,
                                input_filepath_caract,
                                input_filepath_places,
                                input_filepath_veh,
                                output_folderpath):
